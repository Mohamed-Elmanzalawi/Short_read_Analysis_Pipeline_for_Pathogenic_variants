...
for dir in ${temp_dir} ${haplotype_output}/00_bam_files/ ${haplotype_output}/01_bam_after_marks/ ${haplotype_output}/02_marksduplicate_metrics/ \
${haplotype_output}/03_BQSR_report/ ${haplotype_output}/04_after_bqsr_results/ ${output_dir}/01_gvcf/
do
    mkdir -p ${dir} 
done
...
-O ${haplotype_output}/00_bam_files/${NAME}.germline.bam --SORT_ORDER coordinate --TMP_DIR ${temp_dir}

# Mark Duplicates
singularity exec ${gatk_sif} gatk --java-options -Xmx16g \
            MarkDuplicates \
            -I ${haplotype_output}/00_bam_files/${NAME}.germline.bam \
            -O ${haplotype_output}/01_bam_after_marks/${NAME}.germline_dedup.bam \
            -M ${haplotype_output}/02_marksduplicate_metrics/${NAME}.germline_dedup.bam.metrics.txt \
            --TMP_DIR ${temp_dir}
...
singularity exec ${gatk_sif} gatk --java-options -Xmx16g \
            BaseRecalibrator \
            -R ${Ref} \
            -I ${haplotype_output}/01_bam_after_marks/${NAME}.germline_dedup.bam \
            -O ${haplotype_output}/03_BQSR_report/${NAME}.germline_recal.table \
...
singularity exec ${gatk_sif} gatk --java-options -Xmx16g \
            ...
            -R ${Ref} \
            -I ${haplotype_output}/01_bam_after_marks/${NAME}.germline_dedup.bam \
            --bqsr-recal-file ${haplotype_output}/03_BQSR_report/${NAME}.germline_recal.table \
            -O ${haplotype_output}/04_after_bqsr_results/${NAME}.germline_dedup_bqsr.bam


#Run Haplotype Caller
singularity exec ${gatk_sif} gatk --java-options -Xmx16g \
            HaplotypeCaller \
            --input ${haplotype_output}/04_after_bqsr_results/${NAME}.germline_dedup_bqsr.bam \
            --output ${output_dir}/01_gvcf/${NAME}.germline.g.vcf \
            --reference ${Ref} \
            -ERC GVCF \
